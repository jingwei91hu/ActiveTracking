...
from numpy import trace,zeros,eye,einsum,transpose,pad

def bmm(A,B):
    return einsum('ijk,ika->ija',A,B)


def fisher_information_np(mu,cov,d_mu,d_cov):
    inv_cov = inv(cov)
...
def crlb_np(mu,cov,d_mu,d_cov,W=None,c_inv_hat = None):
    f = fisher_information_np(mu,cov,d_mu,d_cov)
